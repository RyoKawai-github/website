...
// 座標の配列clistに座標coが入っているか
const coordinateIn = (clist,co) => {
  return clist.some((c) => c[0]==co[0]&&c[1]==co[1]&&c[2]==co[2]);
}
// 座標の配列clistから座標coを排除する
const coordinateDel = (clist,co) => {
  return clist.filter((c) => !(c[0]==co[0]&&c[1]==co[1]&&c[2]==co[2]));
}
